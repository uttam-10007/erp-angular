...
@Component({
  selector: 'app-sor-estimate',
  templateUrl: './sor-estimate.component.html',
  styleUrls: ['./sor-estimate.component.css']
})
export class SorEstimateComponent implements OnInit {

  
  constructor(private sorService:SorService,private settingService:SettingService,private EstimateService:EstimateService,private hierarchyService:HierarchyService, public mainService: MainService, private _script: ScriptLoaderService, private sanitizer: DomSanitizer, private spinner: NgxSpinnerService, private snackBar: MatSnackBar) { }
  erpUser;
  b_acct_id;
  Obj = { dataArr: [],contractor_prof:0,overhead:0,output_capacity_quantity:1 };
  @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  displayedColumns = ['id', 'remark', 'region_cd','circle_cd', 'activity_code', 'proj_cd', 'prod_cd',
    'bud_cd','est_bud', 'action'];
  datasource;
  allEstimate = [];
  allProjectHier=[];
  allProductHier=[];
  ...
  async getItemList() {
    this.spinner.show()
    var resp = await this.sorService.getListOfItems(this.b_acct_id)
    if (resp['error'] == false) {
  ...
  addline() {
    this.baseitemarr = []
    this.Obj['dataArr'].push({ sor_item_category: '', data: [],item_work:'', item_code: '', desc: '', unit: 0,rate:0, quantity: 0,total:0,pwd_spec:'' })
  }
  deleteline(i) {
    this.baseitemarr = []
  ...
  change(i) {
    ...
    for (let j = 0; j < this.Obj['dataArr'][i]['data'].length; j++) {
      

      if ( this.Obj['dataArr'][i]['data'][j]['item_code'] == this.Obj['dataArr'][i]['item_code']) {

        this.Obj['dataArr'][i]['desc'] = this.Obj['dataArr'][i]['data'][j]['desc']
        this.Obj['dataArr'][i]['unit'] =this.Obj['dataArr'][i]['data'][j]['unit']
        this.Obj['dataArr'][i]['pwd_spec'] =this.Obj['dataArr'][i]['data'][j]['pwd_spec']
        this.Obj['dataArr'][i]['rate'] =Number(this.Obj['dataArr'][i]['data'][j]['rate'])
  ...
  changecategory(i){
    ...
    for (let j = 0; j < this.itemList.length; j++) {
      ...
      if (this.itemList[j]['sor_item_category'] == this.Obj['dataArr'][i]['sor_item_category']) {
        this.Obj['dataArr'][i]['data']=JSON.parse(this.itemList[j]['data'])
      }
    }
...
allprojobj = {}
  ...
  check() {   
    ...
    for (let i = 0; i < this.Obj['dataArr'].length; i++) {
      let amt= this.Obj['dataArr'][i]['rate'] * this.Obj['dataArr'][i]['quantity'] 
      this.Obj['dataArr'][i]['total']=Number(amt.toFixed(2))
      total = total + amt;
    }
    this.Obj['est_bud'] = Number(total.toFixed(2))
  }
  allprodobj = {}
  async getAllProduct(){
    var obj = new Object();
  ...
  async getAllEstimate() {
    var resp = await this.sorService.getsorestimate(this.b_acct_id);
    if (resp['error'] == false) {
...
this.changeProject()
  ...
  async submit() {
    ...
    this.Obj['b_acct_id'] = this.b_acct_id;
     var resp = await this.sorService.createsorestimate(this.Obj);
    if (resp['error'] == false) {
      await this.getAllEstimate();
      this.spinner.hide();
      swal.fire('Success', 'SOR Estimate  Created Successfully!!','success');
    } else {
      this.spinner.hide();
  ...
  async update() {
    ...
    var data = JSON.stringify(this.Obj['dataArr']);
    this.Obj['data'] = data;
    var resp = await this.sorService.updatesorestimate(this.Obj);
    if (resp['error'] == false) {
      await this.getAllEstimate();
      this.spinner.hide();

      swal.fire('Success', 'SOR Estimate Updated Successfully!!','success');
    } else {
      this.spinner.hide();
  ...
  async delete(element) {
    ...
    obj['b_acct_id'] = this.b_acct_id;
    obj['id'] = [element['id']];
    var resp = await this.sorService.deletesorestimate(JSON.stringify(obj));
    if (resp['error'] == false) {
      await this.getAllEstimate();

      this.spinner.hide();
      swal.fire('Success', 'SOR Estimate Deleted Successfully!!','success');
    } else {
      this.spinner.hide();
  ...
  async deleteAll() {
    ...
    if (resp['error'] == false) {
      ...
      this.spinner.hide();
      swal.fire('Success', 'All SOR Estimate  Deleted Successfully!!','success');
    } else {
      this.spinner.hide();
